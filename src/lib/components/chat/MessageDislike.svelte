<script lang="ts">
	...
	let errorMessage = ''
	let bads: string[] = [] // Inicializamos como un array de strings
	let isLoading = true
	...
	const fetchBad = async () => {
		const apiUrl = `${import.meta.env.VITE_API_AI_URL}/api/v1/feedback_types/Bad`
		...
		try {
			...
			if (fetchedBot && Array.isArray(fetchedBot.feedback) && fetchedBot.feedback.length > 0) {
				bads = [...fetchedBot.feedback] // Asignamos el contenido de `feedback`
			} else {
